export_function <- function (uniqueID, moduleDir, outputDir, functionFile, gtool) {
    ...
    # get genotypes
    genotypes <- get_genotypes(uniqueID = uniqueID, request = BRCA_table, gtool = gtool, destinationDir = outputDir)
    ...
    for (snp in rownames(BRCA_table)) {
        ...
        output[[snp]][["normal_snp"]] <- BRCA_table[snp, "normal"]
        output[[snp]][["user_snp"]] <- BRCA_table[snp, "Your genotype"]
